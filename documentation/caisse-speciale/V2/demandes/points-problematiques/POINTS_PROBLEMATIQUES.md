...
| **C.9** | **Conversion sans paramètres actifs** : Lors de la conversion d'une demande en contrat, le système ne vérifie pas qu'un `caisseSettings` **actif** existe pour `caisseType`. Le contrat est créé sans `settingsVersion` fiable. | Contrats incohérents, bonus/pénalités mal appliqués | **Critique** |
...
- **C.9** : Lors de la conversion, **charger le paramètre actif** pour `caisseType` (`getActiveSettings(caisseType)`). Si absent, **bloquer la conversion** et afficher un message clair. Passer `settings.id` en `settingsVersion` au contrat créé.
