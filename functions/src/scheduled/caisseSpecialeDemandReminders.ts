 ...
 */
export async function remindPendingCaisseSpecialeDemands(): Promise<void> {
  console.log('[CS Demand Reminders] Début du job')
      ...
      if (daysPending === 3 || daysPending === 7 || daysPending === 14) {
        ...
        if (demand.memberId) {
          try {
            ...
            console.error(`[CS Demand Reminders] Erreur lors de la récupération du membre ${demand.memberId}:`, error)
          }
        }
...
/**
 * Rappelle aux admins les demandes acceptées non converties
 */
export async function remindApprovedNotConvertedCaisseSpecialeDemands(): Promise<void> {
  console.log('[CS Approved Not Converted Reminders] Début du job')
  ...
  try {
    ...
    console.log(`[CS Approved Not Converted Reminders] ${demandsSnapshot.size} demandes acceptées trouvées`)
      ...
      if (daysSinceApproval === 7 || daysSinceApproval === 14) {
        ...
        if (demand.memberId) {
          try {
            ...
            console.error(`[CS Approved Not Converted Reminders] Erreur lors de la récupération du membre ${demand.memberId}:`, error)
          }
        }
        ...
        const existingNotifications = await db
          ...
          .where('module', '==', 'caisse_speciale')
          .where('entityId', '==', demandDoc.id)
          .where('type', '==', 'demand_approved_not_converted')
        ...
        if (existingNotifications.empty) {
          ...
          notificationsCreated++
          console.log(`[CS Approved Not Converted Reminders] Notification créée pour la demande ${demandDoc.id} (${daysSinceApproval} jours)`)
        }
      }
    }

    console.log(`[CS Approved Not Converted Reminders] ${notificationsCreated} notifications créées`)
  } catch (error) {
    console.error('[CS Approved Not Converted Reminders] Erreur:', error)
    throw error
  }
}
