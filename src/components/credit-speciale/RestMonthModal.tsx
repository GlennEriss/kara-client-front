  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <ModalContent size="sm">
        <ModalHeader
          icon={Calendar}
          title={`Mois de repos – Échéance ${monthNumber}`}
          description={`Enregistrer le mois ${monthNumber} comme mois de repos (aucun paiement, aucune pénalité). Indiquez le motif (maladie, cas de force majeure, etc.).`}
        />
        <ModalBody>
        <form id="rest-month-form" onSubmit={handleSubmit} className="space-y-4">
          <div>
