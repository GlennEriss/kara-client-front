...
import { getMembershipRequestById, updateMembershipPayment } from '@/db/membership.db'
...
export default function SubscriptionList() {
  ...
  return (
                                <Card key={subscription.id} className={`rounded-2xl bg-white shadow-sm transition-all duration-200 hover:shadow-md ${active ? 'border border-emerald-200' : 'border border-gray-100'}`}>
                                    <CardContent className="p-4 md:p-5">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                                            ...
                                            <span className={`flex items-center gap-1.5 text-xs font-semibold ${active ? 'text-emerald-700' : 'text-rose-700'}`}>
                                                <span className={`h-2 w-2 rounded-full shrink-0 ${active ? 'bg-emerald-500' : 'bg-rose-400'}`} />
                                                {active ? 'Actif' : 'Expiré'}
                                            </span>
                                        </div>
