...
import { useRequestEarlyRefundCI } from '@/hooks/caisse-imprevue'
import { useAuth } from '@/hooks/useAuth'
import { useContractPaymentStats } from '@/hooks/caisse-imprevue'
import { toast } from 'sonner'
import { earlyRefundCISchema, defaultEarlyRefundCIValues, WITHDRAWAL_MODES, type EarlyRefundCIFormData } from '@/schemas/caisse-imprevue/early-refund-ci.schema'
import { zodResolver } from '@hookform/resolvers/zod'
import { useForm } from 'react-hook-form'

interface EarlyRefundCIModalProps {
  isOpen: boolean
  onClose: () => void
  contract: ContractCI
}
...
export default function EarlyRefundCIModal({
  ...
  useEffect(() => {
    if (isOpen) {
      const defaultValues = {
        ...
        withdrawalAmount: maxAmount > 0 ? maxAmount : 0,
      }
      reset(defaultValues)
      setValue('withdrawalAmount', maxAmount > 0 ? maxAmount : 0)
      setWithdrawalProofFile(null)
      setDocumentPdfFile(null)
    }
  }, [isOpen, reset, maxAmount, setValue])
  ...
  // Validation du montant par rapport au maximum disponible
  ...
  const onSubmit = async (data: EarlyRefundCIFormData) => {
    if (!user?.uid) {
    ...
    if (data.withdrawalAmount > maxAmount) {
      toast.error(`Le montant ne peut pas dépasser ${maxAmount.toLocaleString('fr-FR')} FCFA (montant total versé)`)
      return
    }
    ...
    try {
      await requestEarlyRefundCIMutation.mutateAsync({
  ...
  const isLoading = requestEarlyRefundCIMutation.isPending
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 py-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2 text-sm flex-1">
                ...
                <div className="grid grid-cols-2 gap-2 text-blue-700">
                  ...
                  <p>
                    Montant total versé : {maxAmount.toLocaleString('fr-FR')} FCFA
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {errors.withdrawalAmount && (
                  <p className="text-xs text-red-500 mt-1">{errors.withdrawalAmount.message}</p>
                )}
          ...
          {/* Section 3 : Récapitulatif (optionnel) */}
          {maxAmount > 0 && (
                ...
                {withdrawalAmount && withdrawalAmount > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-blue-600">
                      {withdrawalAmount.toLocaleString('fr-FR')} FCFA
                    </span>
                  </div>
                )}
                {withdrawalAmount && withdrawalAmount > 0 && maxAmount > 0 && (
                  <div className="flex justify-between border-t pt-2 mt-2">
                    ...
                    <span className="font-semibold text-green-600">
                      {(maxAmount - withdrawalAmount).toLocaleString('fr-FR')} FCFA
                    </span>
                  </div>
                )}
              </div>
