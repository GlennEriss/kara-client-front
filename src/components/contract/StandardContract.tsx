  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-4 lg:p-8 overflow-x-hidden">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* Stats — bande partagée `StatStrip`, commune à toutes les fiches contrat. */}
        <StatStrip
          stats={[
            { title: 'Montant mensuel', value: `${formatAmount(data.monthlyAmount || 0)} FCFA`, accent: true },
            { title: 'Durée (mois)', value: data.monthsPlanned || 0, subtitle: formatContractPeriod(data) },
            { title: 'Nominal payé', value: `${formatAmount(data.nominalPaid || 0)} FCFA` },
        ...
        <PaymentCSModal
          ...
          amountDisabled={!editPayment && (data.caisseType === 'STANDARD' || data.caisseType === 'STANDARD_CHARITABLE')}
                    ...
                    {refunds.length === 0 ? (
                      ...
                      refunds.map((r: any) => {
                        ...
                        const getRefundStatusConfig = (status: string) => {
                          if (isEarlyRefund) {
                            if (status === 'ARCHIVED') {
                              return { bg: 'bg-gray-100', text: 'text-gray-700', border: 'border-gray-200', icon: X, label: 'Archivé' }
                            }
                            return { bg: 'bg-green-100', text: 'text-green-700', border: 'border-green-200', icon: CheckCircle2, label: 'Enregistré' }
                          }
                          switch (status) {
                            case 'PENDING':
                              return { bg: 'bg-yellow-100', text: 'text-yellow-700', border: 'border-yellow-200', icon: Clock, label: 'En attente' }
                            case 'APPROVED':
                              return { bg: 'bg-blue-100', text: 'text-blue-700', border: 'border-blue-200', icon: CheckCircle2, label: 'Approuvé' }
                            case 'PAID':
                              return { bg: 'bg-green-100', text: 'text-green-700', border: 'border-green-200', icon: CheckCircle2, label: 'Payé' }
                            default:
                              return { bg: 'bg-gray-100', text: 'text-gray-700', border: 'border-gray-200', icon: X, label: 'Archivé' }
                          }
                        }
