...
import { CaisseContractsService } from '@/domains/financial/caisse-speciale/contrats/services/CaisseContractsService'
...
export default function ValidateMemberSignedCSModal({
  open,
  onOpenChange,
  contract,
}: Props) {
  ...
  useDocumentCI(contract.memberSignedDocumentId) // prefetch
  ...
  const validateMutation = useMutation({
    mutationFn: async () => {
      ...
      return service.validateMemberSignedCSContract(contract.id, user?.uid ?? 'admin', selectedFile)
    },
    onSuccess: () => {
      toast.success('Contrat validé', {
        description: 'Le contrat doublement signé est maintenant le document officiel.',
      })
      queryClient.invalidateQueries({ queryKey: ['caisse-contract', contract.id] })
  ...
  const rejectMutation = useMutation({
    mutationFn: async () => {
      const service = CaisseContractsService.getInstance()
      return service.rejectMemberSignedCSContract(contract.id, user?.uid ?? 'admin')
    },
    onSuccess: () => {
