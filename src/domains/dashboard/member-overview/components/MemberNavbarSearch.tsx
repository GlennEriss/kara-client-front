...
export function MemberNavbarSearch() {
  ...
  const { data: results = [], isLoading } = useMemberGlobalSearch(
    normalized,
    openDropdown,
  );
  ...
  return (
    <>
      <div
        ref={containerRef}
        className="relative w-full md:w-[320px] md:max-w-[45vw]"
      >
        ...
        <Input
          ...
          className="h-10 rounded-lg border-gray-300 bg-white pl-9 pr-9 text-sm focus-visible:border-[#234D65] focus-visible:ring-[#234D65]/20"
        />
        {query ? (
          <button
            type="button"
        ...
        {openDropdown && normalized.length >= 2 ? (
          <Card className="absolute right-0 z-50 mt-2 w-full border border-gray-100 shadow-md">
